...
function App() {
  ...
  useEffect(() => {
    ...
    // 5. SESSION PERSISTENCE (RESTORE SESSION ON REFRESH)
    const restoreSession = async () => {
        // A. Check for Admin Local Session
        const adminSession = localStorage.getItem('zippy-admin-logged');
        if (adminSession === 'true') {
            setRole(UserRole.ADMIN);
            return;
        }

        // B. Check for Supabase Session
        const { data: { session } } = await supabase.auth.getSession();
        if (session) {
            // Fetch profile to determine role
            const { data: profile } = await supabase.from('profiles').select('role').eq('id', session.user.id).single();
            if (profile && profile.role) {
                // Map DB role to App Role Enum
                if (profile.role === 'PASSENGER') setRole(UserRole.PASSENGER);
                else if (profile.role === 'DRIVER') setRole(UserRole.DRIVER);
                else if (profile.role === 'PROVIDER') setRole(UserRole.PROVIDER);
                else if (profile.role === 'ADMIN') setRole(UserRole.ADMIN);
            }
        }
    };
    restoreSession();
  ...
  if (showSplash) {
    return (
      <div className="fixed inset-0 z-[100] bg-zippy-dark flex items-center justify-center overflow-hidden">
        <img 
  ...
  const handleAdminLogin = (e: React.FormEvent) => {
    ...
    if (adminUser === 'admin' && adminPass === '123admin') {
        localStorage.setItem('zippy-admin-logged', 'true'); // Persist Admin
        setTimeout(() => {
            setRole(UserRole.ADMIN);
            setLoading(false);
  ...
  const handleBackToHome = () => {
    // Clear all persistence on explicit logout
    localStorage.removeItem('zippy-admin-logged');
